...
@:final class Entity implements Disposable
{
	...
	#if khambe_debug @:keep #end public function toString () :String
	{
		return toStringImpl("");
	}
...
#if !display
	@:extern // Inline even in khambe_debug builds
	...
	private static function requireComponentType (componentClass :Expr) :ClassType
	{
		...
		if (path != null) {
			...
			case TInst(ref,_):
				var cl = ref.get();
				if (Context.unify(type, Context.getType("khambe.Component")) && cl.superClass != null) {
					return cl;
				}
			default:
			}
