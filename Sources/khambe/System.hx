...
import khambe.platform.Platform;
import khambe.subsystem.ExternalSystem; // IDEA doesn't support wildcard imports
import khambe.subsystem.KeyboardSystem;
import khambe.subsystem.MotionSystem;
import khambe.subsystem.MouseSystem;
import khambe.subsystem.PointerSystem;
import khambe.subsystem.RendererSystem;
import khambe.subsystem.StageSystem;
import khambe.subsystem.StorageSystem;
import khambe.subsystem.TouchSystem;
...
class System
{
	 ...
	 */
	public static var renderer (get, null) :RendererSystem<kha.Image>;
	...
	/**
	 * Starts up khambe, this should usually be the first thing a game does.
	 */
	public static function init (title :String, width :Int, height :Int, onInitialized :Void -> Void)
	{
		if (!_calledInit) {
			khambe.platform.ManifestBuilder.use();
			_calledInit = true;
			_platform.init(title, width, height, onInitialized);
		}
	}
	...
	{
		#if khambe_debug assertCalledInit(); #end
		return _platform.loadAssetPack(manifest);
	}

	/**
	 * Creates a Logger for printing khambe_debug messages. In Flash, this uses the native trace()
	 * function. In JS, logging goes to the console object. Logging is stripped from non-khambe_debug
	 * builds.
	 */
	...
	private static var _platform :Platform = khambe.platform.kha.KhaPlatform.instance;
