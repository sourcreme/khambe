...
import khambe.util.SignalConnection;
import khambe.util.Value;

using khambe.util.BitSets;
...
class Sprite extends Component
{
	...
	@:allow(khambe) function onPointerDown (event :PointerEvent)
	{
		onHover(event);
		if (_pointerDown != null) {
	...
	@:allow(khambe) function onPointerMove (event :PointerEvent)
	{
		onHover(event);
		if (_pointerMove != null) {
	...
	@:allow(khambe) function onPointerUp (event :PointerEvent)
	{
		switch (event.source) {
		case Touch(point):
			if (_pointerOut != null && _flags.contains(HOVERING)) {
