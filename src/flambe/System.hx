...
class System
{
	...
	{
		if (!_calledInit) {
			flambe.platform.ManifestBuilder.use();
			_calledInit = true;
	...
	{
		#if flambe_debug assertCalledInit(); #end
		return _platform.loadAssetPack(manifest);
	}

	/**
	 * Creates a Logger for printing flambe_debug messages. In Flash, this uses the native trace()
	 * function. In JS, logging goes to the console object. Logging is stripped from non-flambe_debug
	 * builds.
	 */
	...
	private static var _platform :Platform = flambe.platform.kha.KhaPlatform.instance;
