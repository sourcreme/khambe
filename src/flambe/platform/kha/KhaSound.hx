...
class KhaSound extends BasicAsset<KhaSound> implements Sound
{
	...
	@:allow(flambe.platform.kha.KhaPlatform) private static function pauseAll(pause :Bool) : Void
	{
...
class KhaPlayback implements Playback
{
	...
	@:allow(flambe.platform.kha.KhaSound) private function systemPaused(paused :Bool) :Void
	{
		if(paused)
			_channel.pause();
		else
	...
	@:allow(flambe.platform.kha.KhaSound) private static var _playbacks :Array<KhaPlayback> = [];
}
