 ...
 * ```
 */
@:autoBuild(khambe.util.PackageLog.build())
#end
class PackageLog
{
#if macro
	public static function build () :Array<Field>
	{
		...
		var logger = Context.defined("khambe-server")
			? macro khambe.server.Node.createLogger($tag)
			: macro khambe.System.createLogger($tag);

		return Macros.buildFields(macro {
			var public__static__logger :khambe.util.Logger = $logger;
			...
			function inline__public__static__log (level :khambe.util.Logger.LogLevel,
